...
from domain.cleaned_to_redshift.transform_cleaned_data import (
    momentum,
    rolling_returns,
    rsi,
    volatility,
...
def test_rolling_returns(setup):
    ...
    result_returns = (
        result_df.select("rolling_returns").rdd.flatMap(lambda x: x).collect()
    )
...
def test_volatility(setup):
    ...
    result_volatility = (
        result_df.select("parkinson_volatility").rdd.flatMap(lambda x: x).collect()
    )
